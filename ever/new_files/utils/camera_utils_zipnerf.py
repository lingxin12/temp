...
def pixels_to_rays(
  ...
  # ic(camera_dirs_stacked.reshape(-1, 3), pixtocams, pixel_dirs_stacked)
  ...
  if correct_coordinates:
    corr = matmul(camtoworlds[..., :3, :3], xnp.diag(xnp.array([1.0, -1.0, -1.0])))
    vecs = mat_vec_mul(corr, camera_dirs_stacked)
    camera_dirs_stacked = matmul(
        camera_dirs_stacked, xnp.diag(xnp.array([1.0, -1.0, -1.0]))
    )
  else:
    vecs = mat_vec_mul(camtoworlds[..., :3, :3], camera_dirs_stacked)
  ...
  radii = (0.5 * (dx_norm + dy_norm))[Ellipsis, None] * 2 / np.sqrt(12)
